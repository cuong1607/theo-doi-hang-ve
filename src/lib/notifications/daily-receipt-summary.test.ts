...
test("formatDailyReceiptSummaryMessage: multiple suppliers — spec's exact example", () => {
  const s = summary({
    date: "2026-09-25",
    receiptCount: 6,
    totalDelivered: 430,
  ...
  assert.equal(
    ...
    [
      ...
      "- SL nhận: 428",
      "- Chênh lệch: -2",
      "- Giá trị hàng nhận: 25.600.000 đ",
    ].join("\n")
  );
});
...
test("formatDailyReceiptSummaryMessage: VND amount formatting uses thousand separators and 'đ' suffix", () => {
  ...
  assert.ok(formatDailyReceiptSummaryMessage(s).includes("- Giá trị hàng nhận: 1.234.567 đ"));
});
