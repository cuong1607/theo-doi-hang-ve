...
export function SendDailyPaymentSummaryButton({ disabled }: { disabled?: boolean }) {
  ...
  async function handleClick() {
    ...
    setShowDetail(false);
    try {
      const res = await fetch("/api/notifications/daily-payment-summary", { method: "POST" });
      const data = await res.json();
      if (res.ok && data.success) {
        setResult(data);
      } else {
        setResult({ success: false, errorMessage: data.errorMessage ?? "Gửi báo cáo thanh toán thất bại." });
      }
    } catch {
      setResult({ success: false, errorMessage: "Không thể gọi API gửi báo cáo thanh toán." });
    } finally {
      setPending(false);
      router.refresh();
    }
  ...
  return (
    <div className="space-y-2">
      <Button variant="outline" onClick={handleClick} disabled={disabled || pending}>
        {pending ? "Đang gửi..." : "Gửi báo cáo thanh toán hôm nay"}
      </Button>
      ...
      {result && result.success && result.skipped && (
        <p className="flex items-center gap-1.5 text-sm text-muted-foreground">
          ...
          Hôm nay chưa có thanh toán nào — không gửi thông báo.
        </p>
      )}
